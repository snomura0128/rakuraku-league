...
export async function PATCH(
  ...
  try {
    ...
    if (leagueResult.length === 0) {
      return NextResponse.json({ error: 'League not found or insufficient permissions' }, { status: 404 })
    }
    ...
    if (match.league_id !== league.id) {
      return NextResponse.json({ error: 'Match does not belong to this league' }, { status: 403 })
    }
