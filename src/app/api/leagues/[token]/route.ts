...
import { leagues, players, tables, matches } from '@/lib/db/schema'
...
function calculateStandings(players: any[], matches: any[]) {
  ...
  matches.forEach(match => {
    if (match.status === 'completed' && match.winner_id) {
      const player1Index = standings.findIndex(s => s.player.id === match.player1_id)
      const player2Index = standings.findIndex(s => s.player.id === match.player2_id)
  ...
  standings.forEach(standing => {
    standing.win_rate = standing.matches_played > 0 
      ? standing.wins / standing.matches_played 
      : 0
  })
