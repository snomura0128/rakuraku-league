      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardHeader>
            ...
            <CardDescription>{error || "リーグ戦の情報を取得できませんでした"}</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-3">
                ...
                <Badge variant={league.is_admin ? "default" : "secondary"} className="shrink-0">
              ...
              <div className="space-y-2">
                ...
                {league.description && (
                  <p className="text-muted-foreground text-sm md:text-base">{league.description}</p>
                )}
              </div>
...
function MatchMatrix({
  ...
  const isPlayerInPlayingMatch = (playerId: string) => {
    return matches.some(
      (m) => m.status === "playing" && (m.player1_id === playerId || m.player2_id === playerId)
    )
  }
                  ...
                  <td key={player2.id} className="p-0.5 text-center">
                    {player1.id === player2.id ? (
                      ...
                      <MatchCell
                        ...
                        isAdmin={isAdmin}
                        isDisabled={
                          isPlayerInPlayingMatch(player1.id) || isPlayerInPlayingMatch(player2.id)
                        }
...
function MobileMatchCards({
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "pending" | "playing" | "completed">(
    "pending"
  )
        ...
        {[
          {
            ...
            label: "未対戦",
            count: allMatches.filter(({ match }) => match.status === "pending").length,
          },
          {
            key: "playing" as const,
            label: "試合中",
            count: allMatches.filter(({ match }) => match.status === "playing").length,
          },
          {
            key: "completed" as const,
            label: "試合終了",
            count: allMatches.filter(({ match }) => match.status === "completed").length,
          },
                  ...
                  <div className="text-center text-gray-500">
                    {match.status === "playing" &&
                      (match.table_id
                        ...
                        : "試合中")}
                    {match.status === "pending" &&
                      (isDisabled ? "他の試合中のため対戦不可" : "タップして試合開始")}
                  </div>
                )}
              </div>
            </div>
...
function MatchCell({
  ...
  const [checkmarkAnimationData, setCheckmarkAnimationData] = useState<any>(null)
  ...
  const renderCompletedMatch = () => {
    ...
    const animationData = isUIPlayer1Winner ? checkmarkAnimationData : xMarkAnimationData
