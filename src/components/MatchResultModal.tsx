...
export default function MatchResultModal({
  ...
  const handleCompleteMatch = async () => {
    ...
    try {
      ...
      await onUpdateMatch(match.id, "completed", winnerId, dbSetsWonPlayer1, dbSetsWonPlayer2)
      onClose()
    } catch (error) {
                ...
                {allTables.map((table) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isAvailable && <div className="text-sm text-gray-400 mt-1">使用中</div>}
                    </button>
                  )
                })}
              </div>
            ) : (
              <div className="text-center py-4 text-gray-500">台がありません</div>
            )}
