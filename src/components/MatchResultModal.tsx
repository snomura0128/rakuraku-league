...
import type { Match, Player, Table, MatchFormat } from '@/types'
...
interface MatchResultModalProps {
  ...
  onUpdateMatch: (matchId: string, status: string, winnerId?: string, setsWonPlayer1?: number, setsWonPlayer2?: number, tableId?: string) => Promise<void>
}
...
export default function MatchResultModal({
  ...
  const [setsWonPlayer1, setSetsWonPlayer1] = useState(match.sets_won_player1 || 0)
  const [setsWonPlayer2, setSetsWonPlayer2] = useState(match.sets_won_player2 || 0)
  const [selectedTableId, setSelectedTableId] = useState('')
  const [showTableSelection, setShowTableSelection] = useState(match.status === 'pending')
  ...
  const handleTableSelect = async (tableId: string) => {
    setIsSubmitting(true)
    try {
      await onUpdateMatch(match.id, 'playing', undefined, undefined, undefined, tableId)
      onClose()
    } catch (error) {
  ...
  const player1Options = scoreOptions.filter(option => option.winner === player1.id)
  const player2Options = scoreOptions.filter(option => option.winner === player2.id)
  ...
  const handleCompleteMatch = async () => {
    ...
    setIsSubmitting(true)
    try {
      await onUpdateMatch(match.id, 'completed', winnerId, setsWonPlayer1, setsWonPlayer2)
      onClose()
    } catch (error) {
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="text-center">
                <div className="text-lg font-semibold">
                  {match.winner_id === player1.id ? player1.name : player2.name} の勝利
                </div>
