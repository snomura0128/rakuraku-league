    ...
    <div className={cn("flex items-center justify-between mb-8 px-4", className)}>
      {steps.map((step, index) => (
        <div key={step.id} className="flex flex-col items-center relative flex-1">
            ...
            className={cn(
              "w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm mb-2 transition-all duration-300 relative z-10",
              {
                ...
                "bg-primary text-primary-foreground shadow-lg ring-4 ring-primary/20":
                  step.status === "active",
                "bg-muted text-muted-foreground border-2 border-border": step.status === "pending",
              }
            )}
          >
          ...
          {/* Step Label */}
          <span
            className={cn("text-sm text-center transition-colors duration-300 max-w-24", {
              "text-foreground font-medium": step.status === "active",
              "text-muted-foreground": step.status === "completed" || step.status === "pending",
            })}
          >
            {step.title}
          </span>
          ...
          {index < steps.length - 1 && (
            <div
              className={cn(
                ...
                {
                  "bg-orange-500": step.status === "completed",
                  "bg-border": step.status === "active" || step.status === "pending",
                }
              )}
