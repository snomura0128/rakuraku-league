...
export const players = sqliteTable('players', {
  id: text('id').primaryKey(),
  league_id: text('league_id').notNull().references(() => leagues.id, { onDelete: 'cascade' }),
  name: text('name').notNull(),
  created_at: text('created_at').notNull().default(sql`CURRENT_TIMESTAMP`),
})
...
export const matches = sqliteTable('matches', {
  id: text('id').primaryKey(),
  league_id: text('league_id').notNull().references(() => leagues.id, { onDelete: 'cascade' }),
  player1_id: text('player1_id').notNull().references(() => players.id, { onDelete: 'cascade' }),
  player2_id: text('player2_id').notNull().references(() => players.id, { onDelete: 'cascade' }),
