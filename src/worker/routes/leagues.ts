...
app.post("/", async (c) => {
  try {
    ...
    const db = drizzle(c.env.DB, {
      schema: { leagues, matches, players, tables },
    })
    ...
    const tableIds = Array.from({ length: validatedData.table_count }, () => generateId())
    ...
    if (error instanceof z.ZodError) {
      return c.json({ success: false, error: "Invalid input data", details: error.issues }, 400)
    }
...
app.get("/:token", async (c) => {
  try {
    ...
    if (leagueResult.length === 0) {
      return c.json({ success: false, error: "League not found" }, 404)
    }
...
app.patch("/:token/matches", async (c) => {
  try {
    const token = c.req.param("token")
    const body = await c.req.json()
    const { matchId, status, winnerId, setsWonPlayer1, setsWonPlayer2, tableId } = body
    ...
    if (leagueResult.length === 0) {
      return c.json({ error: "League not found or insufficient permissions" }, 404)
    }
    ...
    if (match.league_id !== league.id) {
      return c.json({ error: "Match does not belong to this league" }, 403)
    }

    // Handle table assignment when match starts playing
    if (status === "playing" && tableId) {
      // Check if table is available
      const tableResult = await db.select().from(tables).where(eq(tables.id, tableId)).limit(1)
